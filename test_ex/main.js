...
function displayActions() {
  ...
  document.querySelectorAll(".delete-btn").forEach((btn, index) => {
    btn.addEventListener("click", function () {
      const shouldDelete = confirm("آیا از حذف این اقدام مطمئن هستید؟");
      if (shouldDelete) {
...
fileInput.addEventListener("change", async (event) => {
  ...
  if (file) {
    ...
    formData.append("test_file", file); // مطمئن شوید که کلید اینجا با چیزی که در سرور استفاده شده مطابقت دارد

    try {
      const response = await fetch("http://localhost:3000/run-tests", {
        method: "POST",
        body: formData, // اینجا نیازی به تنظیم `Content-Type` نیست، مرورگر خودش مقدار مناسب را تنظیم می‌کند
      });
